...
TcpCubic::GetTypeId()
{
    static TypeId tid =
        TypeId("ns3::TcpCubic")
            ...
            //               MakeDoubleChecker<double>(0.0))
            .AddAttribute("BetaCubic",
                        "The multiplicative decrease factor for TCP Cubic's congestion window.",
                        DoubleValue(0.7),
                        MakeDoubleAccessor(&TcpCubic::m_beta_cubic),
                        MakeDoubleChecker<double>(0.1, 0.99))

          .AddAttribute("C",
                        "The C factor for TCP Cubic's window growth function.",
                        DoubleValue(0.4),
                        MakeDoubleAccessor(&TcpCubic::m_c),
...
TcpCubic::TcpCubic(const TcpCubic& sock)
    : TcpCongestionOps(sock),
      ...
      m_beta_cubic(sock.m_beta_cubic)
{
    NS_LOG_FUNCTION(this);
}
...
TcpCubic::GetSsThresh(Ptr<const TcpSocketState> tcb, uint32_t bytesInFlight)
{
    ...
    uint32_t ssThresh = std::max(static_cast<uint32_t>(segCwnd * m_beta_cubic), 2U) * tcb->m_segmentSize;
